//
//  WLKTActivityDetailEvaluationStarBtnCell.m
//  wlkt
//
...
#import "WLKTActivityDetailEvaluationStarBtnCell.h"

@interface WLKTActivityDetailEvaluationStarBtnCell ()
...
@property (strong, nonatomic) WLKTActivityCommentStar *data;
@end

@implementation WLKTActivityDetailEvaluationStarBtnCell

- (instancetype)initWithStar:(WLKTActivityCommentStar *)data
{
    self = [super init];
    if (self) {
        _data = data;
...
- (UIButton *)createStarButtonWithStarNumber:(NSString *)number count:(NSString *)count{
    UIButton *b = [UIButton new];
    b.titleLabel.font = [UIFont systemFontOfSize:12 * ScreenRatio_6];
    [b setTitle:[NSString stringWithFormat:@"%@星(%@)", number, count] forState:UIControlStateNormal];
    if ([number isEqualToString:@"全部"]) {
        [b setTitle:[NSString stringWithFormat:@"%@(%@)", number, count] forState:UIControlStateNormal];
    }
