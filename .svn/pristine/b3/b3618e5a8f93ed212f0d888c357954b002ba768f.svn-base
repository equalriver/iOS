...
- (void)loadData{
    ...
    [api startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            WLKTNewsCommentReplyHeadView *v = [[WLKTNewsCommentReplyHeadView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, [self.info.content getSizeWithWidth:ScreenWidth -20 Font:14].height + 100)];
            v.delegate = self;
...
- (void)didSelectedNewsDetailCommentButtonByType:(NewsDetailCommentType)type button:(UIButton *)button indexPath:(NSIndexPath *)indexPath{
    ...
    if (type == NewsDetailCommentReply) {//回复
        if ([button.titleLabel.text isEqualToString:@"回复"]) {
            WLKTNewsDetailCommentReply *r = [[WLKTNewsDetailCommentReply alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight) placeholder:[NSString stringWithFormat:@"回复@%@", self.dataArr[indexPath.row].username] index:0 isPushVC:YES];
//            r.delegate = self;
            [self.view addSubview:r];
        }
        else{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    CGFloat h = 85;
    h += [self.dataArr[indexPath.row].content getSizeWithWidth:ScreenWidth -20 Font:14].height;
    return h;
}
