...
@interface WLKTCDViewController ()<UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate, UIScrollViewDelegate, CDBottomButtonDelegate, WLKTLoginCoordinatorDelegate, CDHeaderCellDelegate, CDEvaluationDelegate, LGPhotoPickerBrowserViewControllerDelegate, LGPhotoPickerBrowserViewControllerDataSource, CDOtherCourseDelegate, CDTeacherCellDelegate>
...
@property (nonatomic) BOOL isShowIntro;
@end
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(introTableViewScrollNoti) name:@"kCourseDetailIntroScrollNoti" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(courseIntroBackToDetailNoti) name:@"kCourseIntroBackToDetailNoti" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(touchStatusBarNoti) name:kTouchStatusBarNoti object:nil];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (!self.isShowIntro) {
        [self.contentScroll setContentOffset:CGPointMake(0, 0)];
    }
...
- (void)introTableViewScrollNoti{
    [self.contentScroll setContentOffset:CGPointMake(0, ScreenHeight - 50 - NavigationBarAndStatusHeight)];
}

- (void)courseIntroBackToDetailNoti{
    [self.introVC.tableView.mj_header endRefreshing];
    [UIView animateWithDuration:0.7 animations:^{
        [self.contentScroll setContentOffset:CGPointMake(0, 0)];
    }];
    self.isShowIntro = false;
...
- (void)tableViewPanAct:(UIPanGestureRecognizer *)sender{
    if (sender.state == UIGestureRecognizerStateEnded) {
        UIScrollView *scrollView = (UIScrollView *)(sender.view);
        CGPoint offset = scrollView.contentOffset;
        CGSize size = scrollView.contentSize;
        if (offset.y + ScreenHeight - 50 - 50 >= size.height) {
        
            [self.introVC.view setOrigin:CGPointMake(0, ScreenHeight - 50 - NavigationBarAndStatusHeight)];
            self.introVC.bottomBtns.hidden = YES;
            self.isShowIntro = YES;
            [self.contentScroll addSubview:self.introVC.view];
...
- (void)didSelectedYouhuiType:(CDHeaderCellYouhuiType)type{
    if (type == CDHeaderCellYouhuiTypeZhe_jian) {
        WLKTCDYouhuiAlert *v = [[WLKTCDYouhuiAlert alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
        v.data = self.data;
        [[UIApplication sharedApplication].keyWindow addSubview:v];
    }
    if (type == CDHeaderCellYouhuiTypeCoupon) {
        WLKTCDCouponAlert *v = [[WLKTCDCouponAlert alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
        v.data = self.data;
        [[UIApplication sharedApplication].keyWindow addSubview:v];
    }
}
...
- (void)didClickLicenseButton{
    WLKTCDRenzhengAlert *v = [[WLKTCDRenzhengAlert alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
    v.data = self.data;
    [[UIApplication sharedApplication].keyWindow addSubview:v];
}

- (void)didClickRuleAlertButton{
    WLKTCDPinAlertView *v = [[WLKTCDPinAlertView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
    [[UIApplication sharedApplication].keyWindow addSubview:v];
}

#pragma mark - CDTeacherCellDelegate
- (void)didClickTeacherItem{
    WLKTCDTeacherAlert *alert = [[WLKTCDTeacherAlert alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
...
#pragma mark - set
- (void)setIsShowIntro:(BOOL)isShowIntro{
    _isShowIntro = isShowIntro;
    [[NSNotificationCenter defaultCenter]postNotificationName:@"kCourseShowIntroNoti" object:nil userInfo:@{@"isShowIntro": @(isShowIntro)}];
...
- (UIScrollView *)contentScroll{
    if (!_contentScroll) {
        _contentScroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - 50 -IphoneXBottomInsetHeight)];
        _contentScroll.backgroundColor = [UIColor whiteColor];
        _contentScroll.delegate = self;
        _contentScroll.showsVerticalScrollIndicator = false;
        _contentScroll.contentSize = CGSizeMake(0, (ScreenHeight - 50) *2);
        _contentScroll.scrollEnabled = false;
    }
    return _contentScroll;
}
- (UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - 50 -IphoneXBottomInsetHeight) style:UITableViewStyleGrouped];
        _tableView.delegate = self;
        _tableView.dataSource = self;
        _tableView.backgroundColor = kMainBackgroundColor;
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        [_tableView.panGestureRecognizer addTarget:self action:@selector(tableViewPanAct:)];
    }
    return _tableView;
}
