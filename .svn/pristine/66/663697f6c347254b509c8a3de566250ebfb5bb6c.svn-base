...
//    NSLog(@"long pressed");
    ...
    UIAlertAction *save = [UIAlertAction actionWithTitle:@"保存图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            UIImageWriteToSavedPhotosAlbum(self.photos[self.currentPage].photoImage, nil, nil, nil);
            if (self.cellScrollView.photoImageView.image) {
...
//    UIAlertAction *share = [UIAlertAction actionWithTitle:@"分享图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
////        [WLKTUMShare shareWithTaget:self title:nil detail:nil urlImage:@"" url:@""];
//    }];
