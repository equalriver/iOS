...
#import "WLKTSchoolAppointment.h"

@interface WLKTSchoolAppointment ()
...
@implementation WLKTSchoolAppointment
...
- (void)confirmBtnAct:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(schoolAppointmentConfirmPhone:detail:success:)]) {
        [self endEditing:YES];
        dispatch_time_t t = dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC * 0.25);
        dispatch_after(t, dispatch_get_main_queue(), ^{
            [self.delegate schoolAppointmentConfirmPhone:self.phoneTF.text detail:self.detailTV.text success:^{
                [UIView viewAnimateDismissWithDuration:0.7 delay:0 target:self.contentView completion:^(BOOL finished) {
                    if (finished) {
                        [self removeFromSuperview];
                    }
...
- (UIView *)contentView{
    if (!_contentView) {
        _contentView = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight - NavigationBarAndStatusHeight, ScreenWidth, 410)];
...
- (UILabel *)noticeLabel{
    if (!_noticeLabel) {
        ...
        [str setAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12], NSForegroundColorAttributeName: KMainTextColor_3} range:NSMakeRange(0, str.length - 4)];
        [str setAttributes:@{NSForegroundColorAttributeName: UIColorHex(f6c447), NSUnderlineStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle], NSFontAttributeName: [UIFont systemFontOfSize:13]} range:NSMakeRange(str.length - 4, 4)];
...
- (UILabel *)phonePlaceholderLabel{
    if (!_phonePlaceholderLabel) {
        ...
        [str setAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12], NSForegroundColorAttributeName: KMainTextColor_9} range:NSMakeRange(0, str.length - 4)];
        [str setAttributes:@{NSForegroundColorAttributeName: UIColorHex(37becc), NSFontAttributeName: [UIFont systemFontOfSize:12]} range:NSMakeRange(str.length - 4, 4)];
...
- (UILabel *)detailPlaceholderLabel{
    if (!_detailPlaceholderLabel) {
        ...
        _detailPlaceholderLabel.font = [UIFont systemFontOfSize:12];
...
- (UILabel *)bottomTagLabel_1{
    if (!_bottomTagLabel_1) {
        ...
        _bottomTagLabel_1.font = [UIFont systemFontOfSize:11];
    }
...
- (UILabel *)bottomTagLabel_2{
    if (!_bottomTagLabel_2) {
        ...
        _bottomTagLabel_2.font = [UIFont systemFontOfSize:11];
    }
