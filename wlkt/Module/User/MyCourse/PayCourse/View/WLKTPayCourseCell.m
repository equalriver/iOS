...
@interface WLKTPayCourseCell ()

@end

@implementation WLKTPayCourseCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier cellContent:(WLKTPayCourseData *)data{
    ...
    if (self) {
        ...
        [self.contentView addSubview:self.payNumIcon];
...
- (void)setCellContent:(WLKTPayCourseData *)data{
    ...
    self.payNumLabel.text = [NSString stringWithFormat:@"交易号：%@", data.payment_trade_no];
...
- (void)makeConstraints{
    ...
    [self.payNumIcon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(12, 12));
        make.left.mas_equalTo(self.contentView).offset(15 * ScreenRatio_6);
        make.top.mas_equalTo(self.tradeNumIcon.mas_bottom).offset(8);
    }];
    [self.payNumLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.payNumIcon.mas_right).offset(5);
        make.centerY.mas_equalTo(self.payNumIcon);
    }];
    [self.separatorView_3 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(ScreenWidth, 0.5));
        make.top.mas_equalTo(self.payNumIcon.mas_bottom).offset(12);
    }];
...
- (UIImageView *)payNumIcon{
    if (!_payNumIcon) {
        _payNumIcon = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"交易"]];
    }
    return _payNumIcon;
}
- (UILabel *)payNumLabel{
    if (!_payNumLabel) {
        ...
        _payNumLabel.textColor = KMainTextColor_3;
    }
    return _payNumLabel;
}
