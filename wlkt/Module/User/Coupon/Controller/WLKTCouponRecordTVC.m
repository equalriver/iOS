...
@interface WLKTCouponRecordTVC ()

@end

@implementation WLKTCouponRecordTVC
...
- (void)myCouponSelectedAct:(NSNotification *)sender{
    if (sender.userInfo[@"index"] && [sender.userInfo[@"index"]  isEqual: @(1)]) {
...
- (void)request{
    WS(weakSelf);
    [WLKTMyCouponTools requestWithStatus:2 success:^(WLKTMyCouponData *data) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    WLKTCouponRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:@"WLKTCouponRecordCell"];
    if (cell == nil) {
        cell = [[WLKTCouponRecordCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"WLKTCouponRecordCell"];
    }
