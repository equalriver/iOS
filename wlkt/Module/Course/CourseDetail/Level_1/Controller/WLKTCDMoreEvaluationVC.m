...
@interface WLKTCDMoreEvaluationVC ()<CDBottomButtonDelegate, UITableViewDelegate, UITableViewDataSource, LGPhotoPickerBrowserViewControllerDataSource, LGPhotoPickerBrowserViewControllerDelegate, CDMoreEvaluationPhotoDelegate, WLKTLoginCoordinatorDelegate>
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    h += [self.dataArr[indexPath.row].content getSizeWithWidth:315 *ScreenRatio_6 Font:16 *ScreenRatio_6].height;
    ...
    if (self.dataArr[indexPath.row].reply.count) {
        h += 65 *ScreenRatio_6;
        h += [self.dataArr[indexPath.row].reply.firstObject.content getSizeWithWidth:290 *ScreenRatio_6 Font:16 *ScreenRatio_6].height;
    }
    return h;
}
...
- (UIView *)headerView{
    if (!_headerView) {
        _headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
        ...
        [iv mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(self.headerView).offset(30 *ScreenRatio_6);
        }];
