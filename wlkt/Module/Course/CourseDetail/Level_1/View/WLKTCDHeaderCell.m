...
@property (strong, nonatomic) UIView *youhuiTempView;
@property (strong, nonatomic) WLKTCDHeaderPinLeftTimeView *pinLeftTimeView;
@property (strong, nonatomic) UIView *license;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self.contentView addSubview:self.youhuiTempView];
...
- (void)setCellData:(WLKTCDData *)data{
    ...
    if (data.license.count) {
        ...
        self.license = _license ? _license : [self createLinceseView:data.license];
    ...
    if (data.courseinfo.have_pg.intValue == 1) {//有拼购
        ...
        self.pinLeftTimeView.data = data;
        [self.contentView addSubview:self.pinLeftTimeView];
        [self.pinLeftTimeView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    if (data.youhui.count || data.yhq.count) {
        [self.youhuiTagLabel removeFromSuperview];
        ...
        [self.youhuiTagLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(self.sep_2).offset(8 *ScreenRatio_6);
        }];
    }
    
    //youhui
    [self.youhuiTempView removeAllSubviews];
    if (data.youhui.count) {
        [data.youhui enumerateObjectsUsingBlock:^(WLKTCDDataYouhui * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx < 2) {
                ...
                [self.youhuiTempView addSubview:v];
    ...
    if (youhuiArr.count) {
        ...
        [self.youhuiTempView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self.youhuiTagLabel.mas_right);
            make.right.mas_equalTo(self.contentView).offset(-10 *ScreenRatio_6);
            make.top.mas_equalTo(self.sep_2.mas_bottom);
            make.height.mas_equalTo(youhuiArr.count * 35 *ScreenRatio_6);
        }];
        ...
        if (youhuiArr.count > 1) {
            ...
            [youhuiArr mas_makeConstraints:^(MASConstraintMaker *make) {
                make.width.centerX.mas_equalTo(self.youhuiTempView);
                make.height.mas_equalTo(35 *ScreenRatio_6);
            }];
        }
        else{
            [youhuiArr mas_makeConstraints:^(MASConstraintMaker *make) {
                make.width.centerX.centerY.mas_equalTo(self.youhuiTempView);
                make.height.mas_equalTo(35 *ScreenRatio_6);
...
- (UIView *)createLinceseView:(NSArray<CDDataLicense *> *)licenses {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (licenses[i].order.integerValue <= 0) {
//            [b setImage:[UIImage imageNamed:@"认证未选中"] forState:UIControlStateNormal];
            continue;
        }
        else{
            [b setImage:[UIImage imageNamed:@"license"] forState:UIControlStateNormal];
            [temp mas_updateConstraints:^(MASConstraintMaker *make) {
                make.right.mas_equalTo(detailBtn.mas_left).offset(-(3 - i) *60 *ScreenRatio_6);
            }];
        }
    ...
    sep.backgroundColor = UIColorHex(cccccc);
    [v addSubview:sep];
...
- (UIView *)createPinView{
    ...
    [title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.mas_equalTo(v).offset(20 *ScreenRatio_6);
...
//    }];
    ...
    [self.rawPriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.priceLabel).offset(-3 *ScreenRatio_6);
...
- (UIView *)sep_1{
    if (!_sep_1) {
        _sep_1 = [UIView new];
        _sep_1.backgroundColor = UIColorHex(eeeeee);
    }
    return _sep_1;
}
- (UIView *)sep_2{
    if (!_sep_2) {
        _sep_2 = [UIView new];
        _sep_2.backgroundColor = UIColorHex(eeeeee);
    }
    return _sep_2;
}
...
- (UILabel *)courseNameLabel{
    if (!_courseNameLabel) {
        ...
        _courseNameLabel.font = [UIFont systemFontOfSize:18 *ScreenRatio_6 weight:UIFontWeightSemibold];
...
- (UIView *)youhuiTempView{
    if (!_youhuiTempView) {
        _youhuiTempView = [UIView new];
        _youhuiTempView.backgroundColor = [UIColor whiteColor];
    }
    return _youhuiTempView;
}
...
- (WLKTCDHeaderPinLeftTimeView *)pinLeftTimeView{
    if (!_pinLeftTimeView) {
        _pinLeftTimeView = [[WLKTCDHeaderPinLeftTimeView alloc]init];
    }
    return _pinLeftTimeView;
}
