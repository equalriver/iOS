...
@interface WLKTBugReportVC ()<TZImagePickerControllerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UITextViewDelegate>
...
@property (strong, nonatomic) UILabel *bugLabel;
@property (strong, nonatomic) PlaceholderTextView *bugTV;
...
@property (copy, nonatomic) NSString *bug;
...
@implementation WLKTBugReportVC
- (instancetype)initWithSchoolname:(NSString *)schoolname
{
    ...
    if (self) {
        ...
        [self.topContentView addSubview:self.bugLabel];
        [self.topContentView addSubview:self.bugTV];
...
- (void)uploadCommentWithPhotos:(NSArray<NSData *> *)photos{
    ...
    NSDictionary *param = @{
                            ...
                            @"describe": self.bug ? self.bug : @"",
                            @"phone": self.phone ? self.phone : @"",
                            RequestMD5String
                            };
    
    //发出请求
    NSString *url = [NSString stringWithFormat:@"%@errors/index", kBaseURL];
...
- (void)makeConstraints{
    ...
    [self.bugLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.topContentView).offset(10 * ScreenRatio_6);
        make.top.mas_equalTo(self.separatorView).offset(30 * ScreenRatio_6);
    }];
    [self.bugTV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(270 * ScreenRatio_6, 100));
        make.left.mas_equalTo(self.bugLabel.mas_right).offset(13 * ScreenRatio_6);
        make.top.mas_equalTo(self.separatorView).offset(15 * ScreenRatio_6);
    }];
    [self.phoneTF mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(130, 30));
        make.top.mas_equalTo(self.bugTV.mas_bottom).offset(15 * ScreenRatio_6);
        make.left.mas_equalTo(self.bugTV.mas_left);
    }];
...
- (UILabel *)bugLabel{
    if (!_bugLabel) {
        _bugLabel = [UILabel new];
        _bugLabel.font = [UIFont systemFontOfSize:12];
        NSString *ts = [NSString stringWithFormat:@"错误描述：*"];
        ...
        _bugLabel.attributedText = str;
    }
    return _bugLabel;
}
- (PlaceholderTextView *)bugTV{
    if (!_bugTV) {
        _bugTV = [[PlaceholderTextView alloc]initWithPlaceholderColor:KMainTextColor_9 font:11 * ScreenRatio_6];
        _bugTV.myPlaceholder = @"出现什么错误啦？(5~200个字)";
        _bugTV.font = [UIFont systemFontOfSize:12 * ScreenRatio_6];
        _bugTV.textColor = KMainTextColor_3;
        _bugTV.layer.borderColor = KMainTextColor_9.CGColor;
        _bugTV.layer.borderWidth = 0.5;
        _bugTV.delegate = self;
    }
    return _bugTV;
}
- (UILabel *)phoneLabel{
...
- (UIButton *)confirmBtn{
    if (!_confirmBtn) {
        ...
        _confirmBtn.titleLabel.font = [UIFont systemFontOfSize:15 * ScreenRatio_6];
        ...
        _confirmBtn.layer.cornerRadius = 3 * ScreenRatio_6;
