...
@protocol SchoolNewsReportTypeDelegate <NSObject>
- (void)didSelectedReportType:(UIButton *)sender;
@end

@interface SchoolNewsReportTypeCell: UICollectionViewCell
@property (strong, nonatomic) UIButton *typeBtn;
@property (assign) NSInteger index;
@property (weak, nonatomic) id<SchoolNewsReportTypeDelegate> delegate;
- (void)setCellData:(NSString *)data index:(NSInteger)index;
@end

@implementation SchoolNewsReportTypeCell
...
@interface WLKTSchoolNewsGoReportVC ()<UICollectionViewDelegate, UICollectionViewDataSource, SchoolNewsReportTypeDelegate>
...
@implementation WLKTSchoolNewsGoReportVC
...
- (void)loadReportType{
    ...
    [api startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
        RequestModel *arr = [RequestModel modelWithJSON:request.responseJSONObject];
...
- (void)confirmBtnAct{
    ...
    if (!self.reportType) {
        [SVProgressHUD showInfoWithStatus:@"未选择举报类型"];
        return;
    }
    WLKTSchoolNewsReportApi *api = [[WLKTSchoolNewsReportApi alloc]initWithNewsId:self.nid content:self.reportTV.text type:self.reportType];
    [api startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest *request) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    SchoolNewsReportTypeCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"SchoolNewsReportTypeCell" forIndexPath:indexPath];
    cell.delegate = self;
...
- (PlaceholderTextView *)reportTV{
    if (!_reportTV) {
        ...
        _reportTV.myPlaceholder = @"新闻内容不符合？虚假信息？(5~200个字)";
...
- (UICollectionView *)typeCV{
    if (!_typeCV) {
        ...
        [_typeCV registerClass:[SchoolNewsReportTypeCell class] forCellWithReuseIdentifier:@"SchoolNewsReportTypeCell"];
    }
    return _typeCV;
}
- (UIButton *)confirmBtn{
