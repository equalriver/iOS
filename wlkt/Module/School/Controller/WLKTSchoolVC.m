...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 2) {//学校介绍
        ...
        if (self.school.schoolinfo.businesshours.length) {
            h += 10 + [self.school.schoolinfo.businesshours getSizeWithWidth:ScreenWidth - 20 Font:13.5].height;
        }
        return h + [self.school.schoolinfo.introduce getSizeWithWidth:ScreenWidth - 20 Font:13.5].height;
    }
...
- (void)schoolBottomButtonDidSelectedButton:(UIButton *)button{
    ...
    if (button.tag == 3) {//预约试听
        WLKTSchoolAppointment *appoint = [[WLKTSchoolAppointment alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight -IphoneXBottomInsetHeight)];
...
- (void)didSelectedPhoneButton{
    if (self.school.schoolinfo.suid.length) {
        [WLKTPolicyPhone policyPhoneWithType:WLKTPolicyPhoneTypeSchool typeId:self.school.schoolinfo.suid];
    }
